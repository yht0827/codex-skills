...
- `$ralph-json`: PRD나 요구사항을 Ralph 스타일 `prd.json` backlog로 변환
- `$ralph-flow`: `prd.json` 기반 장기 작업을 story 단위로 반복 실행
- `$tdd-team`: Team Lead / Red / Green / Refactor TDD 흐름 실행

### Code Reading And Review

- `$code-explore`: 낯선 코드베이스의 구조, entrypoint, 의존성, 테스트 흐름 분석
- `$code-review-graph`: graph 기반 리뷰 컨텍스트와 변경 영향 범위 확인
- `$code-quality`: 코드 품질, 테스트 품질, 보안, 성능, 유지보수 위험 평가
- `$diagnose`: 버그, 실패 테스트, flaky 현상, 회귀를 재현 loop로 좁힘

### Design And Prototyping

- `$open-design`: Open Design 실행, Codex 연결, 디자인 산출물 생성 흐름 안내

### Session And Setup

- `$handoff`: 다음 Codex 세션이 이어받을 handoff 작성
- `$catchup`: 이전 handoff를 실제 파일과 git 상태로 검증
- `$spring-boot-init`: Spring Initializr 기반 작은 Spring Boot 프로젝트 생성
- `$superpowers-flow`: Superpowers 중심 개발 흐름과 Compound 기록 단계 안내
- `$quick-help`: 자주 쓰는 명령만 짧게 확인
- `$guide`: 설치된 스킬과 프롬프트 목록 확인
...
```text
$prd                -> 명확한 기능 PRD 작성
$ralph-json         -> PRD를 prd.json으로 변환
$diagnose           -> 원인이 불명확한 버그를 고치기 전 repro loop 구성
$code-review-graph  -> graph 컨텍스트 또는 변경 영향 범위 확인
$open-design        -> 필요할 때 디자인 산출물 생성 또는 점검
$tdd-team           -> Red / Green / Refactor pane으로 story 하나 구현
$ralph-flow         -> 필요하면 여러 story를 자동 반복 실행
$handoff            -> 세션을 멈추기 전 handoff 작성
$catchup            -> 다음 세션에서 handoff 검증
```
